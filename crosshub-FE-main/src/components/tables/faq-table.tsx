...
import { RemoveFAQDialog } from "@/components/dialogs/remove-faq";
import { ChangeOrderFAQDialog } from "../dialogs/change-order-faq";

const FAQTable = <TData, TValue>({
  columns,
  result,
  caption,
  captionSide = "top",
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-4">
        <ChangeOrderFAQDialog
          key={result.value.length}
          initialItems={table
            .getRowModel()
            .rows.map((row) => row.getValue<FAQ>("title"))}
        />
        <RemoveFAQDialog
          selected={table
            .getSelectedRowModel()
